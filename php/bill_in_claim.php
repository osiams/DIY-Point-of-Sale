<?php
class bill_in_claim extends main{
	public function __construct(){
		...
		$this->bill_claim_sku="";	
		$this->Bi=null;	
		$this->bill_in_class="php/class/bill_in.php";
	}
	public function run(){
		$this->Bi=new bill_in_class();
		$this->Bi->setInType();
		$this->billsinClaimPage($this->group,$this->bill_claim_sku);
	}
	public function fetch(){
		$this->loadClass($this->bill_in_class);
		$this->Bi=new bill_in_class();
		if(isset($_POST["b"])&&$_POST["b"]=="claimfill"){
			$this->fetchBillsinFill();
		}else if(isset($_POST["b"])&&$_POST["b"]=="claimedit"){
	...
	private function xxxxxxxxxxxxxxxxxxxxxxfetchBillsinEdit():void{
		$error="";
	...
	private function xxxxxxxxxxxxxxxxxxxxxxxfetchBillsinUpdate():array{
	...
	private function xxxxxxxxxxxxxxxxxxxxxxxxxfetchBillsinPage($error){
	...
	private function fetchBillsinInsert():array{
		...
		$sql["set"]="SELECT @result:=0,
			...
			@note:=".$note.",
			@bill_claim_sku:=".$pn.",
			@pn_root:=(SELECT IFNULL(`pn_root`,'') FROM `bill_claim` WHERE `sku`=".$pn."),
			@pn_key:='',
			@n:=".$n.",
			@sum:=".$sum.",
			@stkey:='proot',
		...
		$sql["run"]="BEGIN NOT ATOMIC 
			...
			IF LENGTH(@message_error) = 0 THEN
				SET @stkey=(SELECT sku_key FROM  it WHERE sku_root='ceroot');
				SET @pn_key=(SELECT sku_key FROM  partner WHERE sku_root=@pn_root);
				SET @pd_length=JSON_LENGTH(@jspd);
				FOR i IN 0..(@pd_length-1) DO
					INSERT  INTO `bill_in_list`  (
					...
					SELECT @stkey, 'ceroot',@sku,`product`.`sku_key`,`product`.`sku_root`,
						JSON_VALUE(@jspd,CONCAT('$[',i,'].name')),
						`product`.`s_type`,
						@sku,@pn_root,@pn_key,
						IF(`product`.`s_type`='p',JSON_VALUE(@jspd,CONCAT('$[',i,'].n')),NULL),
				...
				IF r__>0 THEN
					INSERT INTO `bill_in`  (
						time_id		,in_type		,sku			,lot_from		,lot_root		,bill_po_sku		,n							,sum,
						pn_root		,pn_key		,bill_no			,bill_type	,user				,note	,
						vat_n			,bill_date,
						r_				,_r			,date_reg) 
					VALUES (
						@time_id	,'cl'				,@sku		,NULL			,@sku		,@bill_claim_sku,@n						,@sum,
						@pn_root	,@pn_key	,@bill_no		,@bill_type	,@user		,@note,
						@vat_n		,@bill_date,
						r__			,__r			,now);
	...
	private function xxxxxxxxxxxxxxxxxxxxxxxxxxxcutPerfix(string $json):string{
		$re=[];
	...
	private function xxxxxxxxxxxxxxxxxxxxxxxxxxxdeleteBillsIn():void{
		$error="";
		$note="";
	...
	private function xxxxxxxxxxxxxxxxxxxxxxxxxxxxdeletePage(string $error,string $note):void{
	...
	private function xxxxxxxxxxxxxxxxxxxxxxxxxxxxeditPage(){
	...
	private function xxxxxxxxxxxxxxxxxxxxxxxxxxxxxwriteContentInBillsinEdit(string $sku,array $dt,bool $editable=true):void{
		//print_r($dt);
	...
	private function billsinClaimPage(string $group="",string $sku_root=""){
		//$this->addDir("?a=bills&amp;b=fill&amp;c=in&amp;&pn_partner=claim&amp;sku_root=".$this->bill_claim_sku,"เคลมสินค้าเข้า ");
		$this->addDir($this->Bi->ita[$this->Bi->in_type]["qurey"]."&amp;b=fill&amp;pn_partner=claim&amp;sku_root=".$this->bill_claim_sku,"เคลมสินค้าเข้า ");
		$this->pageHead(["title"=>htmlspecialchars($this->Bi->ita[$this->Bi->in_type]["text"])." DIYPOS","js"=>["billsin","Bi","form_selects","Fsl","fileupload","Ful"],"css"=>["billsin","form_selects","fileupload"],"run"=>["Fsl"]]);
			...
			$this->writeContentInBillsinClaim($group,$sku_root);
			echo '<br /><p class="c">
	...
	private function writeContentInBillsinClaim(string $group="",string $sku_root=""):void{
		$pn=$this->getPartnerAll();	
		$cp=$this->getClaimAll();	
		$g=$this->loadProductPartner($this->bill_claim_sku);
		$pn_root=$g["pn_root"];
		$pd=$g["pd"];
		$pd_all=$g["pd_all"];
		$payu=(isset($_POST["payu"]))?htmlspecialchars($_POST["payu"]):",defaultroot,";
		$product_arr=[];
		for($i=0;$i<count($g["pd"]);$i++){
			array_push($product_arr,$g["pd"][$i]["product_sku_key"]);
		}
		$product_str="[\"".implode(",,",$product_arr)."\"]";
		$product_str=str_replace(",,","\",\"",$product_str);
		$product=$this->propToFromValue($product_str);
		...
		echo '<form class="form100"  name="billsin" method="post">
			...
			<div><img src="?a=bill58&amp;b=viewclaim&amp;sku='.$sku_root.'" class="imgbill" alt="ใบเสร็จเลขที่ '.$sku_root.'" /></div>
			<div class="billinhead">	
				<div>
		...
		$this->form_pd=new form_selects("product","สินค้า","billsin",$id,$product_list_id);	
		$this->form_pd->writeForm("billsin");
		$this->Bi->writeJsDataEdit($pd,true,$pn_root,$id,$product_list_id);
		...
		<br /><br />
		<input type="button" onclick="Bi.billsinClaimSumit(false)" value="นำเข้าสินค้าเคลม" /></form>';
		...
		if($group=="claim"){
			...
			//print_r($pd);
			$this->Bi->writeJsDataEditPrompt($pd_all,true,$pn_root,$id,$product_list_id);
			//$this->writeJsDataEdit($pd,true,$pn_root,$id,$product_list_id);
			
		}
	}
	private function xxxxxxxxxxxxxxxxxxpageBillsIn():void{
	...
	private function xxxxxxxxxxxxxxxxxxxxxxwriteContentBillsIn():void{
	...
	private function xxxxxxxxxxxxxxxxxxxxxxxgetAllBillsIn():array{
		$re=[];
		$sql=[];
	...
	private function xxxxxxxxxxxxxxxxxxxxview():void{
		$error="";
		if(isset($_GET["sku"])){
	...
	private function xxxxxxxxxxxxxxxxxxxxxxxxxxxviewPageDefault(string $error,string $sku=null):void{	
	...
	private function xxxxxxxxxxxxxxxxxxxxxxwriteContentVeiew(array $dt,string $aut=""):void{
		//print_r($dt);
		$edd=(isset($_GET["ed"]))?$_GET["ed"]:"";
	...
	private function xxxxxxxxxxxxxxxxxgetDataView1(string $sku):array{
	...
	private function loadProductPartner(string $sku_root):array{
		...
		$re=["get"=>[]];
		$sql=[];
		$sql["bill_claim_id"]="SELECT @bill_claim_id:=(
				SELECT `id`
				FROM `bill_claim`
				WHERE `bill_claim`.`sku`=".$sku_root."
			)";
		$sql["partner"]="SELECT @pn_root:=(
				SELECT `pn_root`
				FROM `bill_claim`
				WHERE `bill_claim`.`sku`=".$sku_root."
		) AS `pn_root`";
		$sql["product_claim"]="
			SELECT  
				`product_ref`.`sku` AS `product_sku`,`product_ref`.`sku_key` AS `product_sku_key`,
				`product_ref`.`sku_root`,
				`product_ref`.`name` AS `name`	,`product_ref`.`barcode`	AS `barcode`,
				`product_ref`.`price`,`product_ref`.`cost`,
				IFNULL(`bill_in_list`.`s_type`,'') AS `s_type`,IFNULL(`product_ref`.`vat_p`,0) AS `vat_p`,
				SUM(IF(`bill_in_list`.`s_type`='p',`bill_claim_list`.`n`,`bill_claim_list`.`n_wlv`)) AS `n`,
				SUM(`bill_claim_list`.`n`*(`bill_in_list`.`sum`/IF(`bill_in_list`.`s_type`='p',`bill_in_list`.`n`,`bill_in_list`.`n_wlv`))) AS `sum`,
			...
			LEFT JOIN `unit_ref`
			ON(`bill_in_list`.`unit_sku_key`=`unit_ref`.`sku_key`)
			WHERE `bill_claim_list`.`bill_claim_id`=@bill_claim_id AND `bill_claim_list`.`claim_stat`='s'
			GROUP BY `bill_in_list`.`product_sku_root` ;
		";
		$sql["product"]="SELECT 
				`product`.`name`		,`product`.`sku` AS `product_sku`	,`product`.`barcode` AS `barcode`,
				`product`.`cost`,
				0 AS `balance`,0 AS `sum`,
			...
			LEFT JOIN `unit`
			ON(`product`.`unit`=`unit`.`sku_root`)
			WHERE JSON_SEARCH(`partner`, 'one', @pn_root) IS NOT NULL;
		";
		$se=$this->metMnSql($sql,["partner","product_claim","product"]);
		//print_r($se);//exit;
		if($se["result"]){
			$re=[
				"pd"=>$se["data"]["product_claim"],
				"pd_all"=>$se["data"]["product"],
				"pn_root"=>$se["data"]["partner"][0]["pn_root"]
			];
				for($i=0;$i<count($re["pd"]);$i++){
					$re["pd"][$i]["balance"]=$re["pd"][$i]["n"];
				}			
		}
		return $re;
	}
	...
	private function xxxxxxxxxxxxxxxxxxxxxpayu_jsonToFromValue(string $payu_json):string{
		$t="";
	...
	protected function xxxxxxxxxxxxxxxxxxxxxxxsetPropR(string $prop):string{
		$ar = [];
