...
class sell extends main{
	constructor(){
		...
		this.k={"NumpadMultiply":"*","NumpadAdd":"+","Minus":"-","NumpadSubtract":"-","NumpadDecimal":".",
			...
			"ShiftLeftKeyB":"B","ShiftLeftKeyF":"F",
			"ShiftLeftKeyL":"L","ShiftLeftKeyM":"M",
		}
		this.sco1=""
		this.sco2=""
		this.sct=""
	...
	insertList(bc,n=null){
		...
		}else if(this.id(this.idn).value*1!=0){
			n=(n!==null)?n:this.id(this.idn).value*1
			this.setdateNow()
	...
	loadSellList(time){
		...
		setTimeout(S.setLastWhenLoadSellList,timeout+=100,dt_last,n_last,9)
	}
	...
	loadSellListClearBefore(time){
		this.dt={}
		this.rmc_all(this.id(this.dlist))
	...
	insertNewList(sku_root){
		let n_at=this.id(this.dlist).childNodes.length
		let at=1
		if(n_at>=1){
			at=this.id(this.dlist).lastChild.childNodes[0].innerHTML*1+1//Object.keys(this.dt).length	//this.dpro.childNodes.length+1
		}
		...
		this.end(this.id(this.dlist),[dlist])
	}
